function [accruacy, errorlist, prob_tst, Model,history, history_tst] = ...
    mv_cotraining(Traindata, Testdata, param)
...
for iv=1:nV
   ...
   Model{iv} = NaiveBayes.fit(X_L(:,:,iv),y_L, 'Prior', 'uniform'); 
   ...
   OutlierX = X(history.OutlierXind{1},:,:);
   InlierX = X(history.InlierYind{1},:,:);
...
while( outflag && iter < maxIter)   
    ...
    for iv=1:nV
       ...
       [post,cpre] = Model{iv}.posterior(OutlierX(:,:,iv));
    ...
    for iv= 1:nV
       ...
       outlierInd = setdiff([1:size(OutlierX,1)], inlierInd);     
       ...
       % new Inlier set
       InlierX = X(history.InlierYind{iter},:,:); 
      ...
      if ~isempty(history.OutlierXind{iter})
         ...
         OutlierX = X(history.OutlierXind{iter},:,:);
    ...
    for iv=1:nV      
       ...
       Model{iv} = NaiveBayes.fit(InlierX(:,:,iv),InlierY); 
...
%% make prediction 
    for iv=1:nV
       ...
       [post_tst,cpre_tst] = Model{iv}.posterior(X_Tst(:,:,iv));
